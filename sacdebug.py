...
def sacfunc_to_c(func_name, args=list()):
    ...
    if "::" in func_name:
        ...
        sac_namespace = string.upper(func_name.split("::")[0])
...
class SacVariableWatchpoint(gdb.Breakpoint):
    def __init__(self, spec, func_name):
        super(SacVariableWatchpoint, self).__init__(spec, gdb.BP_WATCHPOINT,wp_class=gdb.WP_WRITE, internal=True, temporary=True)
        self.silent = True
    def stop(self):
        return True
...
class SacDebugCommand(gdb.Command):
    ...
    def __init__(self):
        global sac_func_bps
        super(SacDebugCommand, self).__init__("sacdebug", gdb.COMMAND_SUPPORT)
    ...
    if type(event) is gdb.BreakpointEvent:
        ...
        for i, bp in enumerate(event.breakpoints):
            ...
            if valid_points == len(event.breakpoints):
                print("continueing " + str(valid_points))
                gdb.execute("step")
